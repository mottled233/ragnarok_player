...
void BaiduMusic::search(const QString &keyword, int page)
{
    ...
    //networker->get("https://www.baidu.com");
    //connect(networker,SIGNAL(finished(QNetworkReply*)),this,SLOT(output()));
    //构造请求链接url
...
void BaiduMusic::output()
{
    //qDebug()<<"baidu get url";
}
...
void BaiduMusic::searchReplyFinished()
{
  ...
  //  qDebug()<<"currentPage : "<<currentPage<<" count: "<<pageCount<<" keyword: "<<keyword<<" result: "<<result;
...
//QSting  xml = lyricReply->readAll(); //with some xml structure
...
void BaiduMusic::setinfo(QString songname, QString singer)
{
//    qDebug()<<"===="<<songname<<"----"<<singer;
//    QUrl url = QUrl(("http://box.zhangmen.baidu.com/x?op=12&count=1&title=%1$$%2$$$$").arg(songname).arg(singer));
